...
class UserController extends Controller {
    ...
    public function basic_info_fields(Request $request){
        if($request->api_key){
          $user = User::where('api_key', $request->api_key)->select('id')->first();
          if($user) {

              $service = ServiceApply::where('customer_id', $user->id)->where('status', 0)->pluck('service_id')->toArray();
              $cc_banks = [];
              $pl_banks = [];
              if(in_array(3, $service)){
                  $cc_banks = CreditCardPreferenceBank::where('user_id', $user->id)->pluck('bank_id')->toArray();
              }
              if(in_array(1, $service)){
                  $pl_banks = PersonalLoanPreferenceBank::where('user_id', $user->id)->pluck('bank_id')->toArray();
              }
              $bank = array_unique(array_merge($cc_banks, $pl_banks));
              $fields = [];
              $data = \DB::table('bank_fields')
                ->join('forms', 'forms.id', '=', 'bank_fields.field_id')
                ->select('forms.id', 'forms.name', 'forms.label', 'forms.section')
              ->where('forms.form_type', $request->type)->whereIn('bank_fields.bank_id', $bank)->get(); 
              ...
              if(count($data) != 0){
                ...
                foreach ($data as $value) {
                  if(in_array($value->id, $var)){
                    ...
                    $slide['label'] = $value->label;
                    $fields[] = $slide;
                  }
                }
              }

            return response()->json(['success' => true, 'status' => 200, 'data' => $fields]);
          }
        }
    }
    ...
    public function lead_token(Request $request){
    ...
    public function inertPasw(Request $request){
      $user = User::where('mobile', $request->mobile)->where('email', $request->email)->select('id')->first();
      ...
      if(!empty($user)){
        ...
        return response()->json(['success' => true, 'status' => 200, 'message' => '']); 
      }
...
public function agent_login(Request $request){
    ...
    public function card_type_list(Request $request){
      if($request->api_key){
        ...
        // $user_bank = CreditCardPreferenceBank::where('user_id', $user_id)->pluck('bank_id')->toArray();
        // if($user_bank){
        $card_list = CardTypeBank::where('bank_id', $request->bank_id)->pluck('card_type_id')->toArray();
        $data = CardType::where('status', 1)->whereIn('id', $card_list)->select('name', 'image', 'id')->get();
        $url = route('get-started');
        
          foreach($data as $value) {
            $slide['id'] =   $value->id;
            $slide['name'] = $value->name;
            $slide['image'] = $url.$value->image; 
            $datas[] = $slide;
          }

        // }

        return response()->json(['success' => true, 'status' => 200, 'data' => $datas]);

        } 
      } else {
        return response()->json(['success' => false, 'status' => 201, 'message' => "User not login"]); 
      }
    }
      ...
      if($request->api_key){
        $user = User::where('api_key', $request->api_key)->select('id', 'emirates_id', 'gender')->first();
          ...
          $cus_onboard = CustomerOnboarding::where('user_id', $user_id)->select('nationality', 'date_of_birth', 'passport_photo', 'marital_status', 'cm_type', 'passport_number')->first(); 
            if($cus_onboard){
              if($cus_onboard->date_of_birth != NULL && $cus_onboard->marital_status != NULL){
                    $score += 5;
              }

            if($cus_onboard->passport_number != NULL){
                    $score += 5;
            }
    ...
    public function save_card_type_preference(Request $request){
      if($request->api_key){
        $user = User::where('api_key', $request->api_key)->select('id')->first();
        if($user){
          $user_id = $user->id;
            ...
            return response()->json(['success' => true, 'status' => 200, 'message' => 'Card Type preference have been successfully added']);  
        }
      } 
    }
    ...
    public function services_by_cm_type(Request $request){

      $services = Service::where('status', 1)->select('id', 'name')->orderBy('sort_order', 'ASC')->get();
      $data = [];
      foreach ($services as $value) {
        $slide['id'] = $value->id;
        $slide['name'] = $value->name;

        if($request->cm_type == 2 && $value->id == 1){
           $slide['status'] = false;
        } else if($request->cm_type == 1 && $value->id == 4){
           $slide['status'] = false;
        } else {
           $slide['status'] = true;
        }
    ...
    public function sign_up(Request $request){
      try {
            ...
            $PreRegister =  PreRegister::where('id', $request->id)->where('mobile_status', 1)->select('email', 'mobile')->first();
            if($PreRegister){
              $api_key = $this->generateApiKey();
    ...
    public function profile_image64(Request $request){
      ...
      if($user){
        ...
        $profile_image = 'attach_' . time() . '.png'; //generating unique file name;
        ...
        // if ($attachmant_base != "") {
          ...
          file_put_contents(public_path().'/uploads/user_images/'.$profile_image, base64_decode($attachmant_base));
          $profile_image = '/uploads/user_images/'.$profile_image;
    ...
    public function email_verify(Request $request){
      try {
          ...
          return response()->json(['success' => true, 'status' => 200, 'message' => $message, 'id' => $id]);
            ...
            $inputs['email_status'] = 0; 
            $id = $request->id;
            (new PreRegister)->store($inputs, $id);
            $message = "Email id verification is skip";
            return response()->json(['success' => true, 'status' => 200, 'message' => $message, 'id' => $id]);
        }
    ...
    public function show_product_requested(Request $request){
      if($request->api_key){
        ...
        if($user) {
          ...
          if($datas->expected_monthly_average_spend){
            $data['expected_monthly_average_spend'] = $datas->expected_monthly_average_spend;
          } else {
            $data['expected_monthly_average_spend'] = null;
          }

          if($datas->variable_income){
            $data['variable_income'] = $datas->variable_income;
          } else {
            $data['variable_income'] = null;
          }
      ...
      if($request->api_key){
        ...
        if($user) {
          ...
          if($request->mortgage_loan_amount){ 
            ...
            $inputs['expected_monthly_average_spend'] =  $request->expected_monthly_average_spend;
            ...
            $inputs['variable_income'] =  $request->variable_income;
        ...
        if($request->api_key){
            ...
            if($user) {
              ...
              $user = User::where('id', $user_id)->select('emirates_id', 'emirates_id_back')->first();
                ...
                unset($inputs['emirates_id_front']);
                $inputs['emirates_id_front'] = $emirates_id_front;
                ...
                unset($inputs['emirates_id_back']);
                $inputs['emirates_id_back'] = $emirates_id_back;
    ...
    public function logout(Request $request){
     ...
     public function updateProfile64(Request $request){
      try{   

          if($request->api_key){
           $user = User::where('api_key', $request->api_key)->select('id', 'profile_image', 'emirates_id', 'emirates_id_back')->first();
          if($user) {
          $user_id = $user->id;
          ...
          //       }
            ...
            if(isset($request->profile_image)) {
              ...
              $image = 'attach_' . time() . '.png';
                file_put_contents(public_path().'/uploads/user_images/'.$image, base64_decode($request->profile_image));
              $profile_images = '/uploads/user_images/'.$image;
            ...
            // $coman = ComanInformation::where('user_id', $user->id)->select('id')->first();
            // if($coman){
         ...
         //    $address = Address::where('customer_id', $user->id)->select('id')->first();
         //    if($address){
    ...
    public function updateProfile(Request $request){
      try{   
          ...
          $cm_details = CustomerOnboarding::where('user_id', $user_id)->select('id', 'passport_photo')->first();
            ...
            $inputs = $inputs + [ 'updated_by' => $user->id, 'profile_image' => $profile_images, 'emirates_id_back' => $emirates_id_back, 'emirates_id' => $emirates_id_front ];
          $mob = $inputs['mobile'];
          unset($inputs['mobile']);
          $mob = str_replace("+91","", $mob);
          $mob = str_replace("+971","", $mob);
          $inputs['mobile'] = $mob;
        ...
        try{
          ...
          if($request->api_key){
           $user = User::where('api_key', $request->api_key)->select('id', 'salutation', 'name', 
            'middle_name', 'last_name', 'email', 'mobile', 'profile_image', 'gender', 'date_of_birth', 
            'emirates_id_back', 'eid_number', 'emirates_id')->first();
            $url = route('get-started'); 
            ...
            if($user->emirates_id){
              $data['emirates_id_front'] = $url.$user->emirates_id;
            } else {
              $data['emirates_id_front'] = '';
            }

            if($user->emirates_id_back){
              $data['emirates_id_back'] = $url.$user->emirates_id_back;
            } else {
              $data['emirates_id_back'] = '';
            }

            $result = CustomerOnboarding::where('user_id', $user->id)->select('marital_status', 
            ...
            if($result){
              $data['cm_type'] = $result->cm_type;
            } else {
              $data['cm_type'] = 0;
            }

              $data['marital_status'] = @$result->marital_status; 
              $data['wife_name'] = @$result->wife_name;
              $data['spouse_date_of_birth'] = @$result->spouse_date_of_birth;
              $data['wedding_anniversary_date'] = @$result->wedding_anniversary_date;
              if(@$result->passport_photo){
                  $data['passport_photo'] = $url.$result->passport_photo;
              } else {
                  $data['passport_photo'] = '';
              }
              

              $data['passport_number'] = @$result->passport_number;
              $data['passport_expiry_date'] = @$result->passport_expiry_date;

              $address = Address::where('customer_id', $user->id)->select('residential_address_line_1', 'residential_address_line_2', 'residential_address_line_3', 'residential_address_nearest_landmark', 'residential_emirate', 'residential_po_box', 'resdence_type', 'annual_rent', 'duration_at_current_address')->first();
              
              $data['residential_address_line_1'] = @$address->residential_address_line_1;
              $data['residential_address_line_2'] = @$address->residential_address_line_2;
              $data['residential_address_line_3'] = @$address->residential_address_line_3;
              $data['residential_address_nearest_landmark'] = @$address->residential_address_nearest_landmark;
              $data['residential_emirate'] = @$address->residential_emirate;
              $data['residential_po_box'] = @$address->residential_po_box;
              $data['resdence_type'] = @$address->resdence_type;
              $data['annual_rent'] = @$address->annual_rent;
              $data['duration_at_current_address'] = @$address->duration_at_current_address;

              $coman = ComanInformation::where('user_id', $user->id)->select('account_number', 'bank_name', 'iban_number')->first();
              $data['account_number'] = @$coman->account_number;
              $data['bank_name'] = @$coman->bank_name;
              $data['iban_number'] = @$coman->iban_number;
