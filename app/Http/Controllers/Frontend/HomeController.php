...
public function enter_name(Request $request){
    ...
    public function update_profile(Request $request){
        ...
        // $image1 = $request->file('passport_photo')->get();
        // $image2 = $request->file('emirates_id_front')->get();
        ...
        $image = fopen($request->file('emirates_id_front')->getPathName(), 'r');
        $bytes = fread($image, $request->file('emirates_id_front')->getSize());
        ...
        if(empty($string)) {
            ...
            return redirect()->back()->with('emirates_id_not_valid', 'emirates_id_not_valid');

        } else {
            //$message = 'This photo says ' . $string;

            $em_id = str_replace("-", "", $string);
            if(strpos($em_id, $request->eid_number) !== false) {

           // dd($em_id);

            $before_emid = 'Name: ';
            $result1 = strstr($em_id, $before_emid);
            $eid_number = trim(substr($result1, strlen($before_emid)));
            $before_emid = ' Date of Birth: ';
            $result1 = strstr($eid_number, $before_emid);
            $name_user = substr($eid_number, 0, strpos($eid_number, $result1));

            $before_emid = 'Sex: ';
            $result1 = strstr($em_id, $before_emid);
            $eid_number = trim(substr($result1, strlen($before_emid)));
            $gender = substr($eid_number, 0, 1);  

            $before_emid = 'Date of Birth:';
            $result1 = strstr($em_id, $before_emid);
            $eid_number = trim(substr($result1, strlen($before_emid)));
            $dob = substr($eid_number, 0, 10); 

            $before_emid = '/الهوية رقم ';
            $result1 = strstr($em_id, $before_emid);
            $eid_number = trim(substr($result1, strlen($before_emid)));
            $eid_number = substr($eid_number, 0, 15);
            
            // echo "Emirates Id No: ". $eid_number;
            // echo " DOB: ". $dob;
            // echo " Gender: ". $gender;
            // echo " Name: ". $name_user; exit;

            } else {
                return redirect()->back()->with('emirates_id_number_not_match', 'emirates_id_number_not_match');
            }
        }
        }
        }
    ...
    $user = User::where('id', $user_id)->select('emirates_id', 'profile_image', 'emirates_id_back', 'eid_status')->first();
                ...
                if(isset($inputs['emirates_id_front']) or !empty($inputs['emirates_id_front'])) {
                    ...
                    if($file = $request->hasFile('emirates_id_front')) {
                        ...
                        $fileName = $image_name.$img_name;
                        $image_resize->save(public_path('/uploads/emirates_id/' .$fileName));
                        $img2 = public_path('/uploads/emirates_id/'.$fileName);

                        // $face = FaceDetection::extract(public_path('/uploads/emirates_id/'.$fileName))->driver('imagick');
                        // if($face->found) {
                        // } else {
                        //     return redirect()->back()->with('emirates_not_face', 'emirates_not_face');
                        // }
                    ...
                    $emirates_id_front = @$user->emirates_id;
                    $img2 = public_path($user->emirates_id);
                } 
            ...
            if(isset($inputs['profile_image']) or !empty($inputs['profile_image'])) {
                ...
                if($file = $request->hasFile('profile_image'))  {
                    ...
                    $img1 = public_path('/uploads/user_images/'.$fileName); 

                    // $face = FaceDetection::extract(public_path('/uploads/user_images/'.$fileName)); 
                    //     if($face->found) {
                    //     } else {
                    //         return redirect()->back()->with('profile_not_face', 'profile_not_face');
                    //     }  
                }
                ...
                $img1 =  public_path($user->profile_image);     
            }
        ...
        $similarityScore = $result['FaceMatches'][0]['Similarity'];
        } else {
        $similarityScore = 0;    
        }
       ...
       // dd($similarityScore);
        ...
        if($similarityScore < 50){
            return redirect()->back()->with('profile_emirates_not_match', 'profile_emirates_not_match');
        }
