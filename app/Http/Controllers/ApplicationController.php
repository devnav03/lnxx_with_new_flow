...
class ApplicationController extends Controller {
    ...
    public function update_applications_status(Request $request){
        try{
            ...
            $status_name = ApplicationStatus::where('id', $request->status_id)->select('name')->first();
            $app_id = Application::where('id', $request->app_id)->select('email', 'ref_id', 'name', 'middle_name', 'last_name')->first();

            $email = $app_id->email;
            $subject = "Lnxx application no #".$app_id->ref_id." is ".$status_name->name;

            $data['name'] = $app_id->name;
            $data['middle_name'] = $app_id->middle_name;
            $data['last_name'] = $app_id->last_name;
            $data['status'] = $status_name->name;
            $data['ref_id'] = $app_id->ref_id;

            \Mail::send('email.app_status', $data, function($message) use ($email, $subject){
